...
## Обновление подписки через domain fronting

Если реальный домен подписки заблокирован, подписку можно тянуть через фронт-домен (CDN): TLS-рукопожатие идёт с «разрешённым» доменом (SNI), а реальный бэкенд указывается в HTTP-заголовке `Host`. Настраивать в UI ничего не нужно — fronting включается **самим URL подписки** через фрагмент после `#`:

```
https://www.google.com/sub/TOKEN#?resolve-address=www.google.com&host=zeus-940540118252.us-central1.run.app
```

- Хост в URL (`www.google.com`) — фронт-домен: с ним делается TLS/SNI и к нему идёт соединение.
- `resolve-address` — что резолвить/куда подключаться (домен или готовый IP фронта).
- `host` — реальный бэкенд, который уходит в заголовок `Host` (здесь Google Cloud Run).

Роутер сам распознаёт такой URL и выполняет эквивалент:

```sh
curl --resolve www.google.com:443:<IP> \
     -H "Host: zeus-940540118252.us-central1.run.app" \
     "https://www.google.com/sub/TOKEN"
```

Запрос идёт напрямую (фронт-домен доступен без туннеля), а IP фронта на время запроса добавляется в `psw2_direct`, чтобы обойти ядро. Работает и при ручном обновлении, и в автообновлении (Loop Mode). Обычные подписки без фрагмента `#?...` работают как раньше.

> Fronting работает только на тех CDN, которые не разрывают связь SNI и `Host` (Cloudflare, Google Frontend и ряд других его давно блокируют; Cloud Run за произвольным фронтом — зависит от конкретной схемы провайдера подписки).
